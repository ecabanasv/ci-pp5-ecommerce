...
def book_count_processor(request):
    ...
    category_counts = {category.name: Book.objects.filter(category=category).count() for category in categories}
    ...
    for category in categories:
        ...
        subcategory_counts[category.name] = {subcategory.name: Book.objects.filter(subcategory=subcategory).count() for subcategory in subcategories}
    ...
    if current_category_name:
        try:
            ...
            current_category_count = Book.objects.filter(category=current_category).count()
